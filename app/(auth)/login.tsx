...
const Login: React.FC = () => {
  ...
  const login = async (): Promise<void> => {
    ...
    mutate(
      { data: { name: normalizedUsername, password } },
      {
        onSuccess: async (response: postApiLoginResponse) => {
          try {
            ...
            const userInfo: UserInfoDto = {
              ...loginResponse.req,
              ...(loginResponse.permissionClaims ?? loginResponse.req.permissionClaims
                ? {
                    permissionClaims:
                      loginResponse.permissionClaims ?? loginResponse.req.permissionClaims,
                  }
            ...
            await setSession(loginResponse.token, userInfo);
            setAppErrors([]);
            ...
            if (__DEV__ && sanitizedError.devDetails) {
              console.warn('[Login] failed to store credentials', sanitizedError.devDetails);
            }
            toastService.showError(t('auth.failedToStoreCredentials'), t('auth.loginFailed'));
          }
        },
        onError: (error: unknown) => {
          const sanitizedError = sanitize(error);
          if (__DEV__ && sanitizedError.devDetails) {
            console.warn('[Login] request failed', sanitizedError.devDetails);
          }
  ...
  return (
    ...
    >
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1 }}>
        ...
        >
          <Pressable onPress={() => router.push(ROUTES.root)} className="w-3/5 h-[30%]  ">
