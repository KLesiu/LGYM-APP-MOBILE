...
const ExerciseSeriesInTrainigsList: React.FC<ExerciseSeriesInTrainigsListProps> = ({
  ...
  return (
    <View className="flex flex-col pt-2" style={{ gap: 4 }}>
      ...
      <View className="flex flex-col" style={{ gap: 16 }}>
        {listOfExerciseSeriesInTrainigs && listOfExerciseSeriesInTrainigs.length ? (
          listOfExerciseSeriesInTrainigs.map((x) => (
            <ExerciseSeriesInTrainingElement key={x._id} exerciseSeriesInTraining={x} />
          ))
        ) : (
          <Text className="text-textColor text-xs" style={{ fontFamily: 'OpenSans_300Light' }}>
            {t('exercises.noHistoryAvailable')}
          </Text>
        )}
      </View>
    </View>
