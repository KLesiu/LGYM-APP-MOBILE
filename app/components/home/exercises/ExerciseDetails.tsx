...
const ExerciseDetails: React.FC<ExerciseDetailsProps> = ({ exercise, goBack }) => {
  const { t } = useTranslation();
  const [exerciseSeriesDetails, setExerciseSeriesDetails] = useState<
    ExerciseTrainingHistoryItemDto[]
  >([]);
  const [exerciseRecord, setExerciseRecord] = useState<PossibleRecordForExerciseDto>();

  const [isRecordDialogShown, setIsRecordDialogShown] = useState<boolean>(false);
  const { toggleMenuButton } = useHomeContext();

  const { mutate: getScores, isPending: isScoresLoading } =
    usePostApiExerciseGetExerciseScoresFromTrainingByExercise();

  const { mutate: getRecord } = usePostApiMainRecordsGetRecordOrPossibleRecordInExercise();
  ...
  if (isRecordDialogShown) {
    return <RecordsPopUp offPopUp={toggleRecordDialog} exerciseId={exercise._id || ''} />;
  }
  ...
  return (
    <View className="flex flex-col px-2 pt-4 flex-1 " style={{ gap: 16 }}>
      ...
      <View className="flex flex-col bg-cardColor px-4 py-2 rounded-lg flex-1" style={{ gap: 16 }}>
        <ScrollView>
          <View className="flex flex-col">
            <View className="flex flex-row justify-center">
              <BodyPartImage
                {...(exercise.bodyPart?.name ? { bodyPart: exercise.bodyPart.name } : {})}
                showBig={true}
              />
            </View>
