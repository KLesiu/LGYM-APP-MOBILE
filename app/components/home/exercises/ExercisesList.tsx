...
import { Pressable, View, Text, Switch, FlatList } from 'react-native';
...
const ExercisesList: React.FC<ExercisesListProps> = ({
  ...
  const [currentTypeOfExercises, setCurrentTypeOfExercises] = useState<TypeOfExercises>(
    TypeOfExercises.GLOBAL,
  );
  ...
  return (
    <View className="flex flex-col p-4 flex-1 w-full" style={{ gap: 16 }}>
      <View className="flex flex-row items-center justify-between">
        ...
        <View className="flex flex-row justify-end items-center" style={{ gap: 2 }}>
      ...
      <FlatList
        ...
        data={currentTypeOfExercises === TypeOfExercises.GLOBAL ? filteredGlobalExercises : filteredUserExercises}
        keyExtractor={(exercise, index) => exercise._id || `${exercise.name ?? 'exercise'}-${index}`}
