...
const TrainingPlanDayExerciseHeader: React.FC = () => {
  ...
  const openSearch = async (exerciseName?: string) => {
    if (!exerciseName) return toastService.showError(t('training.exerciseNameRequired'));
    const query = encodeURIComponent(exerciseName);
    const url = `https://www.google.com/search?q=${query}`;

    const supported = await Linking.canOpenURL(url);
    if (supported) {
      // allowlisted: fixed Google search URL, gated by canOpenURL before openURL
      await Linking.openURL(url);
    } else {
      toastService.showError(t('training.cantOpenBrowser'));
    }
  };
  return (
